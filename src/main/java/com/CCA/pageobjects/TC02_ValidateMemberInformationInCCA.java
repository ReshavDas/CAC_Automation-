...
public class TC02_ValidateMemberInformationInCCA extends PageObject
{
	/*
	CCA-TestCase 
	-To capture all members information In CCA Application
	*/
	public TC02_ValidateMemberInformationInCCA(WebDriver bdriver)
	{
		super(bdriver);
	}
	public HashMap<String,String> TestCase2(ValidationType vtype, DataProvider dp) throws Exception
	{
		...
		if(Find.element("My_Work_Assignment_Icon").isEnabled())			//Validate My Member Assignment
		{
			...
			if(Find.element("My_Work_Assignment_Page_Header").getText().trim().equals("My Work Assignments"))
			{
				...
				for(int FirstNameHeader=0;FirstNameHeader<cols.size();FirstNameHeader++)        //Iterate Member Table Columns
				{
					...
					{
						FirstNameFound=true;
						Report.log("First Name Column Is Present In Table", Status.PASS);
				...
				if(FirstNameFound==true)		//To check If First Name Header found
				{
					...
					Integer LastPageNum=Integer.valueOf(Find.element("My_Work_Assignment_Table_Grid").getText().trim().substring((Find.element("My_Work_Assignment_Table_Grid").getText().trim().indexOf("of")+3), Find.element("My_Work_Assignment_Table_Grid").getText().trim().indexOf(",")-1));
					String Total_Members=Find.element("My_Work_Assignment_Table_Grid").getText().trim().substring((Find.element("My_Work_Assignment_Table_Grid").getText().trim().lastIndexOf(" ")+1));
					int r=0;
					...
					{
						Report.log(Total_Members+" Members Are Present In CCA Application", Status.Pass);	//Screenshot
						MemberDataList.put("CCA_Members_Count", Total_Members);
						for(int PageNumber=1;PageNumber<=LastPageNum;PageNumber++)			//Iteration for all Tables one by one
						{
							Report.log("My Work Assignment Page: "+PageNumber, Status.Pass);
							WebElement table2=Find.element("My_Work_Assignment_Table_Data");
							Thread.sleep(1000);
							List<WebElement> rows2=table2.findElements(By.tagName("tr"));		//Iterate rows
							Thread.sleep(1000);
							for(int j=1;j<rows2.size();j++)     //Iteration for all rows in one Table one by one
							{
								r++;
								...
								List<WebElement> cols1=rows2.get(j).findElements(By.tagName("td"));	
								Thread.sleep(1000);
							...
							if(PageNumber!=LastPageNum) 		//To Check Last Table Number
							{
								Find.element("My_Work_Assignment_Table_Next_Page").click();
							}
							else					
							{
								Report.log("Already In Last Page", Status.DONE);
							}
						}
					}
